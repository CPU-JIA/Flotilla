...
>
> 本文档已根据实际 `prisma/schema.prisma` 更新，当前数据库包含 **48个数据模型** 和 **18个枚举类型**。
...
| **PipelineStatus**    | PENDING, RUNNING, SUCCESS, FAILED, CANCELLED    | CI/CD流水线状态  |
| **DataExportFormat**  | JSON, CSV, ZIP                                  | GDPR数据导出格式 |
| **DataExportStatus**  | PENDING, PROCESSING, COMPLETED, FAILED, EXPIRED | 数据导出状态     |
...
| **ApiToken**        | API令牌（Personal Access Token，支持权限范围） |
| **OAuthAccount**    | OAuth第三方账号关联（GitHub/Google/GitLab）    |
| **TwoFactorAuth**   | 双因素认证配置（TOTP）                         |
...
| 模型            | 说明                           |
| --------------- | ------------------------------ |
| **Pipeline**    | 流水线配置（stages、triggers） |
| **PipelineRun** | 流水线执行记录                 |
...
| 模型                | 说明                 |
| ------------------- | -------------------- |
| **WikiPage**        | Wiki页面（层级结构） |
| **WikiPageHistory** | Wiki页面版本历史     |

#### Webhook模块 ✅ 新增

| 模型                | 说明                          |
| ------------------- | ----------------------------- |
| **Webhook**         | Webhook配置（events、secret） |
| **WebhookDelivery** | Webhook投递记录（重试机制）   |
...
// 文件大小验证
fileSize: z.number().max(100 * 1024 * 1024, '单个文件最大100MB')
```
...
| v2.1 | 2025-12-24 | 根据最新代码探索更新：<br>- 模型数量从33个扩展到48个<br>- 枚举类型从17个更新到18个<br>- 新增CI/CD流水线模块（Pipeline、PipelineRun）<br>- 新增Wiki文档模块（WikiPage、WikiPageHistory）<br>- 新增Webhook模块（Webhook、WebhookDelivery）<br>- 新增实时协作模块（CollaborationSession、CollaborationParticipant）<br>- 新增GDPR合规模块（DataExportRequest）<br>- 新增认证增强（ApiToken、OAuthAccount、TwoFactorAuth） | JIA  |
