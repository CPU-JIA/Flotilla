...
class LoadTester {
  ...
  async run() {
    ...
    console.log(`成功: ${this.results.success} (${Math.round((this.results.success / this.results.total) * 100)}%)`);
    console.log(`失败: ${this.results.failed} (${Math.round((this.results.failed / this.results.total) * 100)}%)`);
    console.log(`限流: ${this.results.rateLimited} (${Math.round((this.results.rateLimited / this.results.total) * 100)}%)`);
