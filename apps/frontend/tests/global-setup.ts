  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.user.role !== user.expectedRole) {
        console.warn(`⚠️  Warning: ${user.username} expected role ${user.expectedRole}, got ${data.user.role}`);
      }
      ...
      throw new Error(`Failed to create ${user.username}: ${errorData.message || response.statusText}`);
    }
  } catch (error) {
