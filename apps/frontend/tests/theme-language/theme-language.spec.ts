...
test.describe('主题和语言切换功能测试', () => {
  ...
  test('Dark主题应该应用到Monaco编辑器', async ({ page }) => {
    ...
    if (projectExists) {
      ...
      if (projectId) {
        ...
        if (monacoExists) {
          await monacoContainer.getAttribute('class')
  ...
  test('语言切换应该更新所有UI文本', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /Dashboard|仪表盘/ }).first()).toBeVisible({
      timeout: 2000,
    })
    await expect(page.getByRole('link', { name: /Projects|项目/ }).first()).toBeVisible({
      timeout: 2000,
    })
    await expect(page.getByRole('button', { name: /Logout|退出登录/ }).first()).toBeVisible({
      timeout: 2000,
    })
  })
})
