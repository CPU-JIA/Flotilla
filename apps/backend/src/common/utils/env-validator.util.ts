...
export function validatePath(path: string | undefined): string {
  ...
  if (sanitizedDirs.length === 0) {
    logger.warn(
      'PATH validation failed: no valid directories found, using default',
    );
  ...
  if (sanitized !== path) {
    logger.warn(
      `PATH sanitized: removed ${dirs.length - sanitizedDirs.length} unsafe directories`,
    );
  }
...
export function validateHome(home: string | undefined): string {
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(home)) {
      logger.warn(
        `HOME validation failed: contains dangerous pattern '${pattern}'`,
      );
...
export function validateApiBaseUrl(url: string | undefined): string {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol)) {
      logger.warn(
        `API_BASE_URL validation failed: invalid protocol '${parsed.protocol}'`,
      );
      return defaultUrl;
    }
    ...
    if (!allowedHostnames.includes(hostname) && !isPrivateIp(hostname)) {
      logger.warn(
        `API_BASE_URL validation failed: hostname '${hostname}' not in whitelist`,
      );
      return defaultUrl;
    }
    ...
    logger.warn(
      `API_BASE_URL validation failed: invalid URL format - ${error.message}`,
    );
    return defaultUrl;
  }
}
 ...
 */
export function validateGitEnvironment(
