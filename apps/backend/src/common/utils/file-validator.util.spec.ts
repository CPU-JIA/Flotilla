...
describe('FileValidator', () => {
  ...
  describe('isExtensionAllowed', () => {
    it('应该允许白名单中的文件类型', () => {
      expect(
        isExtensionAllowed('image.png', [FileCategory.IMAGE]),
      ).toBe(true)
      expect(
        isExtensionAllowed('document.pdf', [FileCategory.DOCUMENT]),
      ).toBe(true)
      expect(
        isExtensionAllowed('script.js', [FileCategory.CODE]),
      ).toBe(true)
    })

    it('应该拒绝不在白名单中的文件类型', () => {
      expect(
        isExtensionAllowed('image.png', [FileCategory.DOCUMENT]),
      ).toBe(false)
      expect(
        isExtensionAllowed('archive.zip', [FileCategory.IMAGE]),
      ).toBe(false)
    })
  ...
  describe('validateFileUpload - 正常文件', () => {
    ...
    it('应该验证通过PDF文档', async () => {
      ...
      const file = createMockFile('report.pdf', 'application/pdf', 5120, buffer)
  ...
  describe('validateFileUpload - 攻击向量', () => {
    it('应该拒绝可执行文件', async () => {
      ...
      const file = createMockFile('malware.exe', 'application/x-msdownload', 1024, buffer)
    ...
    it('应该拒绝批处理文件', async () => {
      ...
      const file = createMockFile('malicious.bat', 'application/bat', 50, buffer)
    ...
    it('应该检测文件伪装攻击（EXE伪装成PNG）', async () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('MIME类型不匹配'))).toBe(true)
    })
    ...
    it('应该拒绝超大文件', async () => {
      ...
      const file = createMockFile('huge.pdf', 'application/pdf', 200 * 1024 * 1024, buffer)
    ...
    it('应该拒绝不在白名单中的文件类型', async () => {
      const buffer = createFileSignature('zip')
      const file = createMockFile('archive.zip', 'application/zip', 1024, buffer)
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('不允许的文件类型'))).toBe(true)
    })
  ...
  describe('validateFileUpload - 边界情况', () => {
    ...
    it('应该处理文件名编码问题', async () => {
      ...
      // 模拟Multer的Latin1编码
      const latinEncodedName = Buffer.from('中文文件.txt', 'utf-8').toString('latin1')
