...
describe('Secret Validator', () => {
  ...
  describe('isBase64', () => {
    ...
    it('should return true for valid Base64 strings', () => {
      ...
      expect(isBase64('K7gNU3sdo+OL0wNhqoVWhr3g6s1xYv72ol/pe/Unols=')).toBe(true);
  ...
  describe('validateJwtSecret', () => {
    ...
    it('should warn for non-Base64 secrets with sufficient entropy', () => {
      ...
      const nonBase64Secret = 'aB1$cD2@eF3#gH4!iJ5%kL6^mN7&oP8*qR9(sT0)uV1-wX2_yZ3+aB4=cD5';
      const result = validateJwtSecret(nonBase64Secret);
      expect(result.valid).toBe(true);
      expect(result.suggestions).toBeDefined();
      expect(result.suggestions!.some(s => s.includes('not valid Base64'))).toBe(true);
    });

    it('should warn for entropy below recommended threshold', () => {
      // 创建一个长度足够但熵值略低的密钥（4.0 < entropy < 4.5）
      const mediumEntropySecret = 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJ';
      const result = validateJwtSecret(mediumEntropySecret);
      if (result.entropy && result.entropy < 4.5) {
        expect(result.valid).toBe(true);
        expect(result.suggestions).toBeDefined();
        expect(result.suggestions!.some(s => s.includes('recommended'))).toBe(true);
      }
    });
  ...
  describe('validateJwtSecretOrThrow', () => {
    it('should throw error for invalid secrets', () => {
      expect(() => validateJwtSecretOrThrow(undefined)).toThrow('not configured');
      expect(() => validateJwtSecretOrThrow('tooshort')).toThrow('too weak');
      expect(() => validateJwtSecretOrThrow('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')).toThrow('insufficient entropy');
    });
    ...
    it('should use custom name in thrown errors', () => {
      expect(() => validateJwtSecretOrThrow(undefined, 'CUSTOM_SECRET')).toThrow('CUSTOM_SECRET');
    });
