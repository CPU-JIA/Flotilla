...
interface RaftMessage {
  type: 'RequestVote' | 'AppendEntries' | 'RequestVoteResponse' | 'AppendEntriesResponse'
  requestId: string
  data: any
}
...
export class WebSocketTransport extends EventEmitter implements RaftTransport {
  ...
  constructor(private readonly nodeId: string, portMapping: Record<string, number>) {
    super()
   ...
   * 发送RequestVote RPC
   */
  async sendRequestVote(nodeId: string, request: RequestVoteRequest): Promise<RequestVoteResponse> {
   ...
   * 发送AppendEntries RPC
   */
  async sendAppendEntries(nodeId: string, request: AppendEntriesRequest): Promise<AppendEntriesResponse> {
  ...
  private handleConnection(ws: WebSocket, req: any): void {
    ...
    // 处理连接错误
    ws.on('error', (error) => {
      console.error(`[${this.nodeId}] WebSocket error with ${remoteNodeId}:`, error)
      this.connections.delete(remoteNodeId)
    })
  }
  ...
  private async handleMessage(ws: WebSocket, data: string): Promise<void> {
    try {
      ...
      if (message.type === 'RequestVote') {
        ...
        ws.send(JSON.stringify(responseMessage))
      } else if (message.type === 'RequestVoteResponse' || message.type === 'AppendEntriesResponse') {
        // 处理RPC响应
   ...
   * 发送RPC请求
   */
  private async sendRPC(nodeId: string, type: 'RequestVote' | 'AppendEntries', data: any): Promise<any> {
  ...
  private async createConnection(nodeId: string): Promise<WebSocket> {
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        ...
        ws.on('error', (error) => {
          console.error(`[${this.nodeId}] Connection error to ${nodeId}:`, error)
          this.connections.delete(nodeId)
        })
        ...
        ws.on('message', async (data) => {
          try {
            ...
            console.error(`[${this.nodeId}] Error handling message from ${nodeId}:`, error)
          }
        })
