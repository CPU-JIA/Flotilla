...
describe('TwoFactorService', () => {
  ...
  describe('generateSecret', () => {
    ...
    it('should throw error if userId is empty', async () => {
      await expect(service.generateSecret('')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('generateQRCode', () => {
    it('should generate QR code successfully', async () => {
      const otpauthUrl =
        'otpauth://totp/Flotilla (test@example.com)?secret=JBSWY3DPEHPK3PXP&issuer=Flotilla';
  ...
  describe('enable2FA', () => {
    ...
    it('should throw error for invalid verification code', async () => {
      await expect(
        service.enable2FA(mockUserId, mockSecret, '000000'),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('should throw error for missing parameters', async () => {
      await expect(
        service.enable2FA('', mockSecret, mockToken),
  ...
  describe('verify2FA', () => {
    ...
    it('should throw error if 2FA is not enabled', async () => {
      ...
      await expect(service.verify2FA(mockUserId, mockToken)).rejects.toThrow(
        BadRequestException,
      );
    });
