...
describe('CollaborationService', () => {
  ...
  describe('createSession', () => {
    ...
    it('should throw error if documentId or projectId is missing', async () => {
      await expect(service.createSession('', 'file', 'project-1')).rejects.toThrow(
        'documentId and projectId are required',
      )

      await expect(service.createSession('test.md', 'file', '')).rejects.toThrow(
        'documentId and projectId are required',
      )
    })
  })
