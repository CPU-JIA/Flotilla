...
import { ClusterConfigService, type RaftClusterSettings } from './cluster-config.service'
...
export class RaftClusterService implements OnModuleInit, OnModuleDestroy {
  ...
  async startCluster(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Raft cluster started successfully. Node ID: ${settings.nodeId}`)
